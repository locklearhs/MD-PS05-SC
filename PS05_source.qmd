...
ggplot(hate_crimes, aes(x=trump_support, y=hate_crimes))+
  geom_boxplot()+
  labs(
    title="Hate Crimes per 100k individuals by Trump Support",
    y="Hate crimes per 100k indvs.",
    x="Level of trump support (baseline high)"
  )
...
The higher the trump support, the lower the rate of hate crimes. This is an inverse/negative relationship. This is not what I would have expected.
:::
...
States with high trump support have a predicted average of 0.191 hate crimes per 100k individuals.
:::
...
The model estimates that the number of hate crimes per 100,000 people in states with "low" Trump support is `r round(coef(hate_mod)["trump_supportlow"], 3)`.

```{r}
# Type your code and comments inside the code chunk
T1<- get_regression_table(hate_mod)
T1 [2,2] |>
  pull()
```
:::
...
The model estimates that hate crimes are more frequent in states listed as "low" Trump support, because `r T1[2, 2] |> pull()` represents "low" and it is the highest compared to the baseline and medium Trump support.
:::
...
med_total-high_total

# or

coef(hate_mod)["trump_supportmedium"]
```

Hate crimes in "medium" Trump support states were `r coef(hate_mod)["trump_supportmedium"]` greater than "high" Trump support states.
:::
...
get_regression_points(hate_mod)|>
  distinct(hate_crimes_hat)
```

The three possible fitted values for this model are:
`r get_regression_points(hate_mod)|> distinct(hate_crimes_hat) |>
  pull()|> _[1]`, `r get_regression_points(hate_mod)|> distinct(hate_crimes_hat) |> pull()|> _[2]`, and `r get_regression_points(hate_mod)|> distinct(hate_crimes_hat) |>
  pull()|> _[3]`.
:::
...
The numbers that represent the group means of hate crimes are each the same as the previous fitted values.
:::
...
The five states are `r paste(c("Oregon", "Washington", "Massachusetts", "Minnesota", "Maine"), collapse = ", ")`.
:::
